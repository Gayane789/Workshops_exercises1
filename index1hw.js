...
function userSortByFirstName() {
  console.log(
    userSorted.sort((a, b) => a.name.first.localeCompare(b.name.first))
  );
}
userSortByFirstName(userSorted);
...
//   console.log(customersSalary.reduce((prev, customer) => prev + parseFloat(customer.salary), 0));
// }
// totalSalary(customersSalary);   //601645.8391999999

// v2;
let sumOne = 0;
function totalSalary() {
  customersSalary.forEach((customer) => {
    sumOne += parseFloat(customer.salary);
  });
  return sumOne;
}
totalSalary(customersSalary);
...
let sum = 0;
function sumSalary() {
  for (let i = 0; i < customersSalary.length; i++) {
    sum += parseFloat(customersSalary[i].salary);
  }
  return sum;
}
sum = sumSalary(customersSalary);
console.log(sum); // 601645.8391999999
...
// v1;
function filteredUserByFriendsLength(filterUser) {
  console.log(filterUser.filter((user) => user.friends.length < 8));
}
filteredUserByFriendsLength(filterUser);

// v2;
function filteredUser(filterUser) {
  let result = [];
  for (let i = 0; i < filterUser.length; i++) {
    if (filterUser[i].friends.length < 8) {
      result.push(filterUser[i]);
    }
  }
...
function getUsersAllFriends(mappedUsers) {
  console.log(mappedUsers.map((user) => user.friends));
}
getUsersAllFriends(mappedUsers);
////////////////////////////////////////////////////////////////////////////////////////////////
...
function getUsersSomeProp(mappedUsers) {
  console.log(
...
user.name = "Anush";
user.age = 18;
user.email = "anushikimonyan@gmail.com";
user.greet = function () {
  return "Hi";
};
...
user.greet();
user.updateEmail("anushsimonyan@gmail.com");
...
function countTotalPagesRead() {
  const totalRead = collection.reduce(
    (prev, user) => prev + parseFloat(user.pagesRead),
    0
  );
  return totalRead;
}
...
//     return this.books.push({ id, title, author, pages, isRead });
//   },
...
//       .map(
//         (book) =>
//           ` Id: ${book.id}, Title: ${book.title}, Author: ${book.author}, Pages: ${book.pages}, IsRead: ${book.isRead}`
//       )
//       .join("\n");
//   },

//   addNewBook(id, title, author, pages, isRead) {
//     return this.books.push({ id, title, author, pages, isRead });
//   },
...
// librarySystem.addBook(1, "The Great Gatsby", "F. Scott Fitzgerald", 180, true);
// librarySystem.addBook(2, "To Kill a Mockingbird", "Harper Lee", 281, false);
// console.log(librarySystem.books);
// librarySystem.addNewBook(3, "1984", "George Orwell", 328, false);

// console.log(librarySystem.books);
// console.log(librarySystem.listBooks());
// console.log(librarySystem.getBookById(2));
// librarySystem.updateBook(1, {
//   id: 4,
//   title: "1984",
